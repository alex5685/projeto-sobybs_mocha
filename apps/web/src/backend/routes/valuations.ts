...
// Helper function to call DeepSeek for AI-enhanced valuation
async function generateAIValuation(businessData: any, apiKey: string) {
  const deepseek = new OpenAI({
    baseURL: "https://api.deepseek.com",
    apiKey: apiKey,
  });

  const prompt = `Você é um especialista em valuation de empresas. Analise os seguintes dados de uma empresa brasileira e forneça uma avaliação detalhada.

Dados da Empresa:
- Ramo de Atividade: ${businessData.ramo_atividade || "Não informado"}
- Segmento: ${businessData.segmento || "Não informado"}
- Tempo de Mercado: ${businessData.tempo_mercado || "Não informado"}
- Faturamento Mensal: ${businessData.faturamento_mensal || "Não informado"}
- Despesas Fixas: ${businessData.despesas_fixas || "Não informado"}
- Número de Funcionários: ${businessData.num_funcionarios || "Não informado"}
- Receita Recorrente: ${businessData.receita_recorrente || "Não informado"}
- Concentração de Clientes: ${businessData.concentracao_clientes || "Não informado"}
- Tendência de Crescimento: ${businessData.tendencia_crescimento || "Não informado"}
- Contratos de Longo Prazo: ${businessData.contratos_longo_prazo || "Não informado"}
- Dependência do Proprietário: ${businessData.dependencia_proprietario || "Não informado"}

Por favor, forneça:
1. Uma estimativa de valor justo da empresa (em reais)
2. Três principais riscos identificados (categoria, descrição, impacto, probabilidade)
3. Três recomendações prioritárias para aumentar o valor (título, prioridade, impacto estimado, prazo)
4. Um score de atratividade de 1 a 10

Retorne APENAS um JSON válido no seguinte formato:
{
  "valor_estimado": 500000,
  "riscos": [
    {"categoria": "Operacional", "descricao": "...", "impacto": "alto|médio|baixo", "probabilidade": "alta|média|baixa"}
  ],
  "recomendacoes": [
    {"titulo": "...", "prioridade": "alta|média|baixa", "impacto_estimado": "alto|médio|baixo", "prazo": "curto|médio|longo"}
  ],
  "score_atratividade": 7
}`;
  ...
  try {
    ...
    if (!valuation) {
      ...
      if (c.env.DEEPSEEK_API_KEY) {
        aiResult = await generateAIValuation(details, c.env.DEEPSEEK_API_KEY);
      }

      let valorEstimado: number;
      let riscos: any[];
      let recomendacoes: any[];
      let scoreAtratividade: number;

      if (aiResult) {
        // Use AI-generated values
        valorEstimado = aiResult.valor_estimado || 100000;
        riscos = aiResult.riscos || [];
        recomendacoes = aiResult.recomendacoes || [];
        scoreAtratividade = aiResult.score_atratividade || 5;
      } else {
        // Fallback to simple calculation
        ...
        valorEstimado = faturamentoAnual * 2.5;
        ...
        scoreAtratividade = Math.min(10, Math.max(1, Math.round(5 + (valorEstimado / faturamentoAnual))));
      }
      ...
      // Add more risks for higher tier plans
      if (planType === "silver" || planType === "gold") {
        riscos.push(
      ...
      // Add more recommendations for higher tier plans
      if (planType === "silver" || planType === "gold") {
        recomendacoes.push(
