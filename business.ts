...
app.post("/:id/quick-valuation", authMiddleware, async (c) => {
  ...
  try {
    ...
    const faturamentoStr = String(business.faturamento_mensal || '');
    const despesasStr = String(business.despesas_fixas || '');
    
    const faturamento = faturamentoStr ? parseFloat(faturamentoStr.replace(/[^\d,]/g, '').replace(',', '.')) : null;
    const despesas = despesasStr ? parseFloat(despesasStr.replace(/[^\d,]/g, '').replace(',', '.')) : null;
    ...
    let multiplicadorMin = 2.0;
    let multiplicadorMax = 3.5;
    ...
    const valorImoveisStr = String(business.valor_imoveis || '');
    const valorFrotaStr = String(business.valor_frota || '');
    
    const valorImoveis = valorImoveisStr ? parseFloat(valorImoveisStr.replace(/[^\d,]/g, '').replace(',', '.')) : 0;
    const valorFrota = valorFrotaStr ? parseFloat(valorFrotaStr.replace(/[^\d,]/g, '').replace(',', '.')) : 0;
    const ativosIncluidos = valorImoveis + valorFrota;

    // Calculate valuation range
    const valorMinimo = (lucroLiquidoAnual * multiplicadorMin) + ativosIncluidos;
    const valorMaximo = (lucroLiquidoAnual * multiplicadorMax) + ativosIncluidos;
