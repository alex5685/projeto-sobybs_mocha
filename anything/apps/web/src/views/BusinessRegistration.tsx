...
export default function BusinessRegistration() {
  ...
  // If URL has ?businessId=xxx, we're editing that specific business.
  // If not, this is a brand-new registration.
  const editBusinessId = searchParams.get('businessId') ?? '';
  const isEditing = !!editBusinessId;
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // Only pre-fill and set businessId if we're editing a specific business from the URL
        if (editBusinessId) {
          setBusinessId(editBusinessId);
          const detailRes = await fetch(`/api/business/${editBusinessId}`, {
            credentials: 'include',
          });
          if (detailRes.ok) {
        ...
        // If NOT editing (new registration), we leave the form blank — no pre-fill.
  ...
  const handleNext = async () => {
    ...
    try {
      const res = await fetch('/api/business/registration', {
        ...
        credentials: 'include',
        // Always include business_id if we already have one so backend updates the right record
        body: JSON.stringify({ ...formData, business_id: businessId || undefined }),
      });
      if (res.ok) {
        const data = (await res.json()) as { businessId: string };
        // Save the returned businessId so subsequent steps update the same record
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-foreground mb-3">
            {isEditing ? 'Editar Empresa' : 'Cadastro de Nova Empresa'}
          </h1>
