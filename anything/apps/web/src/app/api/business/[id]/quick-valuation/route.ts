...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Check if an existing valuation is still valid for the CURRENT business data
    const [existing] =
      await sql`SELECT * FROM quick_valuations WHERE business_id = ${id} ORDER BY "createdAt" DESC LIMIT 1`;

    if (existing && !forceRefresh) {
      // Compare key financial fields — if data changed, regenerate
      const currentFaturamento = parseBRL(bd.faturamento_mensal);
      const currentDespesas = parseBRL(bd.despesas_fixas);
      const currentLucro = Math.max(0, currentFaturamento - currentDespesas);
      const currentSegmento = (bd.segmento ?? '').toLowerCase().trim();

      const savedLucro = Number(existing.lucro_liquido_mensal_estimado ?? 0);
      const savedSegmento = String(existing.segmento ?? '')
        .toLowerCase()
        .trim();

      // Regenerate if lucro changed by more than 5% or segmento changed
      const lucroChanged =
        savedLucro === 0 || Math.abs(currentLucro - savedLucro) / Math.max(savedLucro, 1) > 0.05;
      const segmentoChanged = savedSegmento !== currentSegmento;

      if (!lucroChanged && !segmentoChanged) {
        // Data hasn't changed — return the cached valuation
        return Response.json({ valuation: existing });
      }

      // Data changed — delete the stale valuation so we regenerate below
      await sql`DELETE FROM quick_valuations WHERE business_id = ${id}`;
    } else if (existing && forceRefresh) {
      // Force refresh requested — delete all existing valuations for this business
      await sql`DELETE FROM quick_valuations WHERE business_id = ${id}`;
    }
