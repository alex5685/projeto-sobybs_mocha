...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Carrega dados atuais da empresa ANTES de checar cache
  ...
  const currentFaturamento = parseBRL(bd.faturamento_mensal as string);
  const currentDespesas = parseBRL(bd.despesas_fixas as string);
  const currentLucroLiq =
    parseBRL(bd.lucro_liquido_mensal as string) ||
    Math.max(0, currentFaturamento - currentDespesas);
  const currentSegmento = String(bd.segmento ?? '')
    .toLowerCase()
    .trim();

  // Verifica se já existe valuation e se ainda é válido para os dados atuais
  if (!forceNew) {
    ...
    if (existing) {
      ...
      // Compare snapshot de dados financeiros com o atual
      const snapshot = val.valuation_data?.data_snapshot as
        | Record<string, number | string>
        | undefined;
      if (snapshot) {
        const savedFaturamento = Number(snapshot.faturamento ?? 0);
        const savedLucro = Number(snapshot.lucro_liquido ?? 0);
        const savedSegmento = String(snapshot.segmento ?? '')
          .toLowerCase()
          .trim();

        const faturamentoChanged =
          savedFaturamento === 0 ||
          Math.abs(currentFaturamento - savedFaturamento) / Math.max(savedFaturamento, 1) > 0.05;
        const lucroChanged =
          savedLucro === 0 ||
          Math.abs(currentLucroLiq - savedLucro) / Math.max(savedLucro, 1) > 0.05;
        const segmentoChanged = savedSegmento !== currentSegmento;

        if (!faturamentoChanged && !lucroChanged && !segmentoChanged) {
          // Dados não mudaram — retorna cache
          return Response.json({
            valuation: { ...val.valuation_data, revisions_count: val.revisions_count, id: val.id },
          });
        }
      } else {
        // Valuation antigo sem snapshot — retorna mas vai regenerar na próxima revisão
        // Para garantir dados corretos, forçamos a regeneração
        console.log('Valuation sem snapshot de dados — regenerando com dados atuais...');
      }

      // Dados mudaram ou sem snapshot — remove valuation antigo e regenera
      await sql`DELETE FROM valuations WHERE business_id = ${id} AND type = 'complete'`;
    }
  } else {
    // Refresh forçado — remove todos os valuations existentes
    await sql`DELETE FROM valuations WHERE business_id = ${id} AND type = 'complete'`;
  }
  ...
  // Adiciona snapshot dos dados que foram usados para gerar este valuation
  const dataSnapshot = {
    faturamento: currentFaturamento,
    lucro_liquido: currentLucroLiq,
    despesas: currentDespesas,
    segmento: currentSegmento,
  };

  const valuationDataWithSnapshot = { ...valuationData, data_snapshot: dataSnapshot };

  // Persiste
  const valuationId = crypto.randomUUID();
  await sql`
    INSERT INTO valuations (id, business_id, type, valuation_data, revisions_count)
    VALUES (${valuationId}, ${id}, 'complete', ${JSON.stringify(valuationDataWithSnapshot)}, 0)
  `;

  return Response.json({
    valuation: { ...valuationDataWithSnapshot, id: valuationId, revisions_count: 0 },
  });
}
