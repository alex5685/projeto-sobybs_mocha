...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await sql(
      `UPDATE faqs SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`,
      values
    );

    if (result.length === 0) {
      return Response.json({ error: 'FAQ não encontrado' }, { status: 404 });
    }

    return Response.json({ faq: result[0] });
  } catch (error) {
    console.error('Error updating FAQ:', error);
    return Response.json({ error: 'Erro ao atualizar FAQ' }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json({ error: 'Erro ao deletar FAQ' }, { status: 500 });
  }
}
